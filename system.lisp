...
(defmacro defsys-new (name (&body letargs) &rest body)
  (let ((entities (gensym)))
    `(progn
...
(defmacro defsys (name grouping required &body body)
  "Define a new system."
  (let ((entities (gensym)))
    `(progn
       (setf (gethash ',name (ecs-systems *ecs*))
...
(defun collect-system-entities (system)
  ...
  (loop :with rs = (required-components system)
        ...
        collect
        (loop
          :for (id . e) :in (alexandria:hash-table-alist (ecs-entities *ecs*))
          :for c = (components e)
          :when (or (not r)
                    (and (listp r) (all r c))
                    (and (eq r :none) (not c))
  ...
  (when (system-processing system)
    (let ((result))
      (apply #'metatilities:map-combinations
             (lambda (&rest x) (setf result (apply #'%do-entities system x)))
